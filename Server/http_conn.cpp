...
bool http_conn::read(){
    if(m_read_idx >= READ_BUFFER_SIZE){
        cout_info("m_read_idx >= READ_BUFFER_SIZE");
        return false;
    }
    int bytes_read = 0;
    while(true){
...
/*分析目标文件属性，将其映射到内存地址 m_file_address处*/
...
bool http_conn::write(){
    ...
    while(1){
        ...
        if(bytes_to_send <= bytes_have_send){
            /*发送HTTP 响应成功，根据HTTP请求中的Connection字段决定是否立即关闭连接*/
            unmap();
            if(m_linger){
                init();
                modfd(m_epollfd, m_sockfd, EPOLLIN);
